...
from aayu.runtime.renderers.web_renderer import RenderNode
...
class Interpreter:
    ...
    def __init__(self, vm):
        self.vm = vm
        self.node_stack = []
        from aayu.runtime.ui.render_tree import RenderTree
    ...
    def _throw_exception(self, exc):
        from aayu.runtime.vm.exceptions import AayuException, InternalException
    ...
    def run(self):
        ...
        while True:
            ...
            if opcode == Opcode.HALT:
                ...
                idx = self.vm.decoder.fetch16(self.vm.registers.ip + 1)
                self.vm.registers.ip += 3
                val = self.vm.call_stack.current().locals[idx]
                self.vm.value_stack.push(val)
                ...
                self.vm.call_stack.current().locals[idx] = val
                ...
                a = self.vm.value_stack.pop()
                if b == 0:
                    from aayu.runtime.vm.exceptions import RuntimeException
                ...
                # Retrieve function metadata from the target
                name = None
                locals_cnt = 0
                for fname, fmeta in getattr(self.vm, "functions_meta", {}).items():
                    if fmeta.get("bytecode_offset") == target:
                        name = fname
                        locals_cnt = fmeta.get("locals", 0)
                        break
                        
                from aayu.runtime.vm.frame import CallFrame
                self.vm.call_stack.push(CallFrame(name or "component", self.vm.registers.ip + 3, locals_cnt))
                ...
                from aayu.runtime.vm.frame import CallFrame
                self.vm.call_stack.push(CallFrame(name or "function", self.vm.registers.ip + 3, locals_cnt))
                ...
                if not token:
                    from aayu.runtime.vm.exceptions import AuthenticationException
                ...
                if self.vm.call_stack.depth() > 0:
                    ...
                    # Backward compatibility for components (to be phased out with real closures)
                    if frame.function_name == "component":
                        if hasattr(self.vm, 'state_scopes') and self.vm.state_scopes:
                            self.vm.state_scopes.pop()
                        if self.node_stack:
                            self.node_stack.pop()
                    self.vm.registers.ip = frame.return_ip
                    if frame.return_ip == -1:
                        return False
                else:
                    return False
