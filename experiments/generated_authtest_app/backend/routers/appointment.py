...
def create_appointment(request: Request, item: AppointmentCreate, db: Session = Depends(get_db)):
...
def update_appointment(request: Request, item_id: str, item: AppointmentUpdate, db: Session = Depends(get_db)):
    db_item = db.query(Appointment).filter(Appointment.id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
...
def delete_appointment(request: Request, item_id: str, db: Session = Depends(get_db)):
    db_item = db.query(Appointment).filter(Appointment.id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
    db.delete(db_item)
