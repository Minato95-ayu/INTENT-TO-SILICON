...
def create_course(request: Request, item: CourseCreate, db: Session = Depends(get_db)):
...
def update_course(request: Request, item_id: str, item: CourseUpdate, db: Session = Depends(get_db)):
    db_item = db.query(Course).filter(Course.id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
...
def delete_course(request: Request, item_id: str, db: Session = Depends(get_db)):
    db_item = db.query(Course).filter(Course.id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
    db.delete(db_item)
