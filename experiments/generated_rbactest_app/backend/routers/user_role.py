...
def create_user_role(request: Request, item: UserRoleCreate, db: Session = Depends(get_db), _=Depends(require_permission('create'))):
...
def update_user_role(request: Request, item_id: str, item: UserRoleUpdate, db: Session = Depends(get_db), _=Depends(require_permission('update'))):
    db_item = db.query(UserRole).filter(UserRole.user_id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
...
def delete_user_role(request: Request, item_id: str, db: Session = Depends(get_db), _=Depends(require_permission('delete'))):
    db_item = db.query(UserRole).filter(UserRole.user_id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
    db.delete(db_item)
