...
export default function UserRoleList() {
  ...
  const fetchItems = async () => {
    ...
    const res = await api.get(`/user_role?${params.toString()}`);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
        <Link to="/user_role/new">
          <button>Add New UserRole</button>
        </Link>
        ...
        <tbody>
          {items.map(item => (
            <tr key={item.id}>
              ...
              <td>{item.user_id}</td>
              <td>
                <Link to={`/user_role/edit/${item.id}`}>
                  <button>Edit</button>
                </Link>
