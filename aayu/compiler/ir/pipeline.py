...
from aayu.compiler.semantic.nodes import (
    ...
    SemanticIfNode, SemanticWhileNode, SemanticForNode, SemanticBinaryOpNode, SemanticModelDeclNode,
...
from aayu.compiler.ir.mir import MIRNode, MIRInstruction
...
class IRPipeline:
    ...
    def _semantic_to_hir(self, node):
        if isinstance(node, SemanticStateDeclNode):
            ...
            if node.name == "show":
                # Ensure we extract the HIR node for the value to print
                val_hir = self._semantic_to_hir(node.args[0]) if getattr(node, 'args', []) else HIRLoadConst("")
                return HIRPrint(val_hir)
            ...
            from aayu.compiler.ir.hir import HIRArrayNode
        ...
        elif isinstance(node, SemanticWhileNode):
            from aayu.compiler.ir.hir import HIRWhile
            cond_hir = self._semantic_to_hir(node.condition)
            if isinstance(cond_hir, HIRPrint): cond_hir = HIRLoadConst(cond_hir.value)
            body_hir = []
            ...
            return HIRWhile(cond_hir, body_hir)
            ...
            return HIRLoadConst(node.value)
            ...
            for a in node.args:
                ...
                arg_hirs.append(h)
            from aayu.compiler.ir.hir import HIRClosure
    ...
    def _hir_to_mir(self, hir, mir_list: list):
        if isinstance(hir, HIRStateDecl):
            ...
            if hir.name == "print":
                print(f"[DEBUG MIR] Compiling PRINT for {hir.name}")
                mir_list.append(MIRInstruction("PRINT_STACK", []))
            elif "." in hir.name or hir.name in ["len", "type", "float", "int"]:
            ...
            for field in hir.fields:
                ...
                for rule in field.rules:
                    ...
                    args_vals = []
                    for a in rule.args:
                        from aayu.compiler.semantic.nodes import SemanticLiteralNode
