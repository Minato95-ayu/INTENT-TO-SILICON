...
export default function RolePermissionList() {
  ...
  const fetchItems = async () => {
    ...
    const res = await api.get(`/role_permission?${params.toString()}`);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
        <Link to="/role_permission/new">
          <button>Add New RolePermission</button>
        </Link>
        ...
        <tbody>
          {items.map(item => (
            <tr key={item.id}>
              ...
              <td>{item.role_id}</td>
              <td>
                <Link to={`/role_permission/edit/${item.id}`}>
                  <button>Edit</button>
                </Link>
