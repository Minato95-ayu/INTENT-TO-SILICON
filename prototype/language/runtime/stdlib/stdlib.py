...
class StdLib:
    def __init__(self, vm):
        ...
        from .collections.list import register_list_stdlib
        from .collections.map import register_map_stdlib
        from .reflect_lib import register_reflect_lib
        
        self.registry = StdLibRegistry()
        
        register_list_stdlib(self.registry)
    ...
    def populate_globals(self, globals_dict):
        ...
        # Add all registered stdlib functions
        for name, func in self.registry.functions.items():
            globals_dict[name] = NativeFunctionValue(name, func)
    ...
    def http_route(self, path: str, arg2: str, arg3: str = None):
        if arg3 is None:
            ...
            method = arg2
            handler_name = arg3
        self.vm.routes[path] = {"method": method, "handler": handler_name}
    ...
    def http_serve(self, port: int, handler_name: str = None):
        ...
        class AayuVMHTTPRequestHandler(BaseHTTPRequestHandler):
            def handle_request(self):
                ...
                handler_bc = vm_instance.globals.get(target_handler_name)
                if not handler_bc and vm_instance.memory.locals_stack:
                    handler_bc = vm_instance.memory.locals_stack[0].get(target_handler_name)
                ...
                try:
                    ...
                    param_name = handler_bc.bytecode.parameters[0] if (hasattr(handler_bc, 'bytecode') and handler_bc.bytecode.parameters) else "req"
                    sub_vm.run(handler_bc.bytecode if hasattr(handler_bc, 'bytecode') else handler_bc, initial_locals={param_name: req_map})
                    ...
                    if isinstance(val, AayuJSONResponse):
                        ...
                        result = val.stringify() if hasattr(val, 'stringify') else str(val)
                        content_type = "text/html; charset=utf-8"
                        status_code = 200
    ...
    def auth_guard_session(self, req_param=None):
