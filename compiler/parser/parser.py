...
class Parser:
    ...
    def _parse_statement(self):
        ...
        if self._match(TokenType.SYMBOL, "@"):
            ...
            while True:
                ...
                from compiler.ast.nodes import DecoratorNode
        ...
        if self._match(TokenType.KEYWORD, "await"):
            from compiler.ast.nodes import AwaitNode
        ...
        if self._match(TokenType.KEYWORD, "useTheme"):
            ...
            from compiler.ast.nodes import UseThemeNode
        ...
        if self._match(TokenType.KEYWORD, "bind"):
            ...
            from compiler.ast.nodes import BindNode
        ...
        if self._match(TokenType.KEYWORD, "navigate"):
            ...
            from compiler.ast.nodes import NavigateNode
        ...
        if self._match(TokenType.KEYWORD, "animate"):
            ...
            from compiler.ast.nodes import AnimateNode
        ...
        if self._check(TokenType.IDENTIFIER) and self._peek().value in lifecycle_hooks:
            ...
            from compiler.ast.nodes import LifecycleNode
    ...
    def _parse_theme_declaration(self):
        ...
        from compiler.ast.nodes import ThemeNode
    ...
    def _parse_validate(self):
        ...
        from compiler.ast.nodes import ValidateNode
        return ValidateNode(line=line, column=col, fields=fields)
        
    def _parse_return_statement(self):
        line, col = self._previous().line, self._previous().column
        value = self._parse_expression()
        from compiler.ast.nodes import ReturnNode
        return ReturnNode(line=line, column=col, value=value)
        
    def _parse_if_statement(self, is_elif=False):
        line, col = self._previous().line, self._previous().column
    ...
    def _parse_for_statement(self):
        ...
        from compiler.ast.nodes import ForNode
    ...
    def _parse_try_statement(self):
        line, col = self._previous().line, self._previous().column
        try_block = []
        while not self._is_at_end() and not self._check(TokenType.KEYWORD, "catch") and not self._check(TokenType.KEYWORD, "finally") and not self._check(TokenType.KEYWORD, "end"):
        ...
        if self._match(TokenType.KEYWORD, "catch"):
            ...
            while not self._is_at_end() and not self._check(TokenType.KEYWORD, "finally") and not self._check(TokenType.KEYWORD, "end"):
        ...
        self._consume(TokenType.KEYWORD, "Expect 'end' at the end of try block.", value="end")
        from compiler.ast.nodes import TryNode
    ...
    def _parse_throw_statement(self):
        line, col = self._previous().line, self._previous().column
        value = self._parse_expression()
        from compiler.ast.nodes import ThrowNode
        return ThrowNode(line=line, column=col, value=value)

    def _parse_rethrow_statement(self):
        line, col = self._previous().line, self._previous().column
        from compiler.ast.nodes import RethrowNode
    ...
    def _parse_model_declaration(self, decorators=None):
        ...
        while not self._is_at_end() and not self._check(TokenType.SYMBOL, "}"):
            ...
            while self._match(TokenType.SYMBOL, "@"):
                ...
                from compiler.ast.nodes import ModelAttributeNode
            ...
            from compiler.ast.nodes import ModelFieldNode
        ...
        from compiler.ast.nodes import ModelDeclNode
    ...
    def _parse_route_declaration(self):
        ...
        while not self._is_at_end() and not self._check(TokenType.KEYWORD, "end"):
            ...
            from compiler.ast.nodes import MethodNode
        ...
        from compiler.ast.nodes import RouteNode
    ...
    def _parse_primary(self):
        ...
        if self._match(TokenType.NUMBER):
            ...
            from compiler.ast.nodes import ArrayNode
        ...
        while True:
            if self._match(TokenType.SYMBOL, "["):
                ...
                from compiler.ast.nodes import SubscriptNode
