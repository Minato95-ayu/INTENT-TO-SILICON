...
from compiler.errors import AAYURuntimeError
...
def test_parser_panic_node():
    ...
    from compiler.ast.nodes import PanicNode
    source = 'panic "fatal".'
    lexer = Lexer(source)
    parser = Parser(lexer.tokenize(), filename="<test>")
    ast = parser.parse()
...
def test_parser_assert_node():
    ...
    from compiler.ast.nodes import AssertNode
...
def test_parser_try_catch_finally():
    ...
    from compiler.ast.nodes import TryNode, CatchNode, FinallyNode
    source = '''
    try.
        print("body").
    catch (e).
