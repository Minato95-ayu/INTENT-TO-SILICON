...
class SemanticAnalyzer:
    ...
    def analyze(self, ast: ProgramNode) -> SemanticProgramNode:
        ...
        if needs_auth and not has_auth_model:
            raise CompilerError("Semantic Error: Found action with @auth_required but no model with @auth_password field exists in the application.", ast.line, ast.column, "")
    ...
    def _analyze_action_decl(self, node: ActionDeclarationNode):
        ...
        from compiler.semantic.nodes import SemanticDecoratorNode
        sem_decorators = []
    ...
    def _analyze_action_call(self, node: ActionCallNode):
        ...
        if node.name and node.name[0].isupper():
            from compiler.semantic.nodes import SemanticWidgetNode
    ...
    def _analyze_array(self, node):
        from compiler.semantic.nodes import SemanticArrayNode
    ...
    def _analyze_try(self, node: TryNode):
        ...
        from compiler.semantic.nodes import SemanticTryNode
    ...
    def _analyze_throw(self, node: ThrowNode):
        ...
        from compiler.semantic.nodes import SemanticThrowNode
