...
def register_networking_lib(registry: StdLibRegistry):
    def fn_mock(args, vm):
        return StringValue("Mock implementation for networking")
        
    # Register basic API methods
    for method in ['connect', 'listen', 'send', 'receive', 'close']:
        registry.register(f"networking::{method}", fn_mock)
