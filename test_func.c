...
AayuValue _aayu_fn_main_logic(void) {
    AayuValue _aayu_test_str = make_str("H");
    AayuValue _aayu_result = _aayu_fn_is_alpha(_aayu_test_str);
    if (aayu_is_truthy(aayu_eq(_aayu_result, make_num(1)))) {
        aayu_print(make_str("It is a valid alphabet/underscore character!"));
    }
    if (aayu_is_truthy(aayu_eq(_aayu_fn_is_alpha(make_str("9")), make_num(1)))) {
        aayu_print(make_str("Wait, 9 shouldn't be alpha."));
    }
    aayu_print(make_str("Done checking!"));
    return make_null();
}
