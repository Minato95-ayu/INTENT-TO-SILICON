...
try:
    ...
    from aayu.compiler.bytecode.encoder import BytecodeEncoder, Opcode
    ...
    old_step = vm.interpreter.step
    log = []
    def traced_step():
        ip = vm.registers.ip
        op = vm.decoder.bytecode[ip]
        if op == Opcode.BUILD_WIDGET:
            w_type = vm.decoder.fetch16(ip+1)
            log.append(f"IP {ip} BUILD_WIDGET type {w_type}")
        else:
            log.append(f"IP {ip} Opcode {op}")
        return old_step()
    vm.interpreter.step = traced_step
