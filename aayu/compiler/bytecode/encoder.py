...
from aayu.compiler.bytecode.constant_pool import ConstantPool
from aayu.compiler.bytecode.header import BinaryHeader
...
class EncodedProgram:
    ...
    def serialize_to_binary(self) -> bytes:
        ...
        out.extend(struct.pack("<BB", 1, 0))
        
        # Constant Pool
        cp_size = len(self.constant_pool.entries)
        out.extend(struct.pack("<I", cp_size))
        for entry in self.constant_pool.entries:
            if entry.type == "INT":
                out.append(0)
                out.extend(struct.pack("<q", int(entry.value))) # 8 bytes
            elif entry.type == "FLOAT":
                out.append(1)
                out.extend(struct.pack("<d", float(entry.value)))
            elif entry.type == "STRING":
                out.append(2)
                s_bytes = str(entry.value).encode('utf-8')
                out.extend(struct.pack("<I", len(s_bytes)))
                out.extend(s_bytes)
            elif entry.type == "BOOL":
                out.append(3)
                out.append(1 if entry.value else 0)
            elif entry.type == "DICT":
                out.append(4)
                # serialize as json string
                s_bytes = json.dumps(entry.value).encode('utf-8')
                out.extend(struct.pack("<I", len(s_bytes)))
                out.extend(s_bytes)
            else:
                out.append(2) # fallback string
                s_bytes = str(entry.value).encode('utf-8')
                out.extend(struct.pack("<I", len(s_bytes)))
                out.extend(s_bytes)
                
        # Bytecode size and instructions
        out.extend(struct.pack("<I", len(self.bytecode)))
...
class BytecodeEncoder:
    ...
    def _encode_print(self, node: LIRNode):
        """PRINT [value] → value_expr + PRINT"""
        if node.operands:
            value_node = node.operands[0]
            if isinstance(value_node, LIRNode):
                self._encode_node(value_node)
            else:
                value_idx = self.pool.add(value_node)
