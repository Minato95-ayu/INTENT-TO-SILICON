...
from compiler.frontend.ir import Opcode, Instruction, Bytecode
...
class AAYUCompiler:
    ...
    def visit_ExtensionDeclNode(self, node):
        # Extensions are purely semantic for Phase 5.6 and do not generate bytecode
        pass
    ...
    def visit_MapLiteralNode(self, node):
        for key, val in node.elements:
            self.visit(key)
            self.visit(val)
        self._emit(Opcode.MAKE_MAP, len(node.elements))

    def visit_UnaryExpressionNode(self, node):
        self.visit(node.right)
        if node.operator in ('-', 'MINUS'):
            # Implement negation (e.g., multiply by -1 or use a NEG opcode if it exists)
            # For now, we can push 0 and swap then SUB to simulate NEG if NEG doesn't exist,
            # or just push -1 and MUL. Let's push -1 and MUL.
            idx = self._add_constant(-1)
            self._emit(Opcode.LOAD_CONST, idx)
            self._emit(Opcode.MUL)
        elif node.operator in ('not', 'NOT'):
