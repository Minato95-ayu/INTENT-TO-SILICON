...
def create_role(request: Request, item: RoleCreate, db: Session = Depends(get_db)):
...
def update_role(request: Request, item_id: str, item: RoleUpdate, db: Session = Depends(get_db)):
    db_item = db.query(Role).filter(Role.id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
...
def delete_role(request: Request, item_id: str, db: Session = Depends(get_db)):
    db_item = db.query(Role).filter(Role.id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
    db.delete(db_item)
