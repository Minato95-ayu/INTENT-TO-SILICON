...
def create_product_order(request: Request, item: ProductOrderCreate, db: Session = Depends(get_db)):
...
def update_product_order(request: Request, item_id: str, item: ProductOrderUpdate, db: Session = Depends(get_db)):
    db_item = db.query(ProductOrder).filter(ProductOrder.product_id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
...
def delete_product_order(request: Request, item_id: str, db: Session = Depends(get_db)):
    db_item = db.query(ProductOrder).filter(ProductOrder.product_id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
    db.delete(db_item)
