...
def create_product(request: Request, item: ProductCreate, db: Session = Depends(get_db)):
...
def update_product(request: Request, item_id: str, item: ProductUpdate, db: Session = Depends(get_db)):
    db_item = db.query(Product).filter(Product.id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
...
def delete_product(request: Request, item_id: str, db: Session = Depends(get_db)):
    db_item = db.query(Product).filter(Product.id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
    db.delete(db_item)
