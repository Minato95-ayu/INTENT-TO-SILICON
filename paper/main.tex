...
\begin{itemize}
    \item Resolved via Active Disambiguation: 62.0\%
    \item Out of Vocabulary (OOV) Rate: 37.2\%
    \item Unresolved Ambiguity (Safe Halt): 0.8\%
\end{itemize}
...
\begin{itemize}
    ...
    \item Emotion Detection Accuracy: 44.0\% (successfully mapping subjective emotional statements to the correct psychological category and subsequently to structural UX patterns).
\end{itemize}
...
\subsection{8.1 Continuous Active Learning & Personalized Pattern Recognition (v0.7)}
...
\subsection{8.2 Automated Dataset Scaling via Social Media Mining (v0.11)}
...
Rather than manual curation, an extraction script filters raw internet "noise" for exact frustration phrases (e.g., \texttt{"dhoondhte dhoondhte chidiya ud jaye"}) and maps them to the appropriate Pain Point category (e.g., \texttt{Navigation / UI Confusion}). To maintain the Zero Hallucination policy, these mined patterns are isolated in a proposed state until validated by a human researcher, ensuring the core dictionary scales accurately without accumulating semantic garbage.
