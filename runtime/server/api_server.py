...
from runtime.server.crud_engine import CrudEngine, ValidationError
from runtime.server.openapi import OpenAPIGenerator
...
class AAYUAPIHandler(BaseHTTPRequestHandler):
    ...
    def _handle_crud(self, method, model_name, record_id, action, query_params):
        ...
        if meta.get("secure", True):
            ...
            if not token:
                return self._send_json(self._format_response(False, error="Unauthorized: Missing token"), 401)
    ...
    def _handle_action(self, action_name):
        ...
        try:
            ...
            expected_stack_depth = self.vm.value_stack.depth() - len(args_schema)
            
            self.vm.call_action_by_name(action_name)
            
            if self.vm.value_stack.depth() > expected_stack_depth:
                ret_val = self.vm.value_stack.pop()
                return self._send_json(self._format_response(True, data=ret_val))
            else:
                return self._send_json(self._format_response(True, data=None))
