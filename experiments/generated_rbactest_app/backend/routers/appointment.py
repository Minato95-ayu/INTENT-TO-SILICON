...
def read_appointment_list(request: Request, page: int = 1, size: int = Query(20, ge=1, le=100), search: Optional[str] = None, sort: Optional[str] = None, order: Optional[str] = 'asc', db: Session = Depends(get_db), _=Depends(require_permission('read'))):
...
def read_appointment(request: Request, item_id: str, db: Session = Depends(get_db), _=Depends(require_permission('read'))):
...
def create_appointment(request: Request, background_tasks: BackgroundTasks, item: AppointmentCreate, db: Session = Depends(get_db), _=Depends(require_permission('create'))):
    ...
    db.add(AuditLog(id=str(uuid.uuid4()), timestamp=datetime.utcnow(), action='create', entity_name='appointment', entity_id=getattr(db_item, 'id', ''), request_id=req_id, user_id=usr_id))
    db.commit()
    db.refresh(db_item)
...
def update_appointment(request: Request, background_tasks: BackgroundTasks, item_id: str, item: AppointmentUpdate, db: Session = Depends(get_db), _=Depends(require_permission('update'))):
    ...
    req_id = getattr(request.state, 'request_id', 'unknown')
    usr_id = getattr(request.state, 'user_id', None) if hasattr(request.state, 'user_id') else getattr(getattr(request.state, 'user', None), 'id', None)
    db.add(AuditLog(id=str(uuid.uuid4()), timestamp=datetime.utcnow(), action='update', entity_name='appointment', entity_id=item_id, request_id=req_id, user_id=usr_id))
    db.commit()
    db.refresh(db_item)
