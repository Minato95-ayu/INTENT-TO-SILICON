...
def register_reflect_lib(registry: StdLibRegistry):
    def fn_mock(args, vm):
        return StringValue("Mock implementation for reflect")
        
    # Register basic API methods
    for method in ['type_of', 'methods', 'properties', 'invoke']:
        registry.register(f"reflect::{method}", fn_mock)
