...
def read_role_list(request: Request, page: int = 1, size: int = Query(20, ge=1, le=100), search: Optional[str] = None, sort: Optional[str] = None, order: Optional[str] = 'asc', db: Session = Depends(get_db)):
...
def read_role(request: Request, item_id: str, db: Session = Depends(get_db)):
...
def create_role(request: Request, background_tasks: BackgroundTasks, item: RoleCreate, db: Session = Depends(get_db)):
...
def update_role(request: Request, background_tasks: BackgroundTasks, item_id: str, item: RoleUpdate, db: Session = Depends(get_db)):
    db_item = db.query(Role).filter(Role.id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
...
def delete_role(request: Request, background_tasks: BackgroundTasks, item_id: str, db: Session = Depends(get_db)):
    db_item = db.query(Role).filter(Role.id == item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail='Not found')
    db.delete(db_item)
